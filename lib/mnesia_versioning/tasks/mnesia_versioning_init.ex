...
defmodule Mix.Tasks.MnesaVersioning.Init do
  @moduledoc """
  ...
  def run(_) do
    if @topology_provider == :required_setting do
      Logger.error "MnesiaVersioningInit - To use the Noizu.MnesiaVersioning library you must specify a topology_provider option in the noizu_mnesia_versioning config section. For more details @see mnesa_versioning/doc/config.md"
  ...
  def setup_versioning_tables(nodes) do
    npids = for (n <- nodes) do
      Node.spawn(n, Mix.Tasks.MnesaVersioningInit, :wait_for_init, [self()])
      receive do
        :wait_mode -> :wait_mode
      end
      pid
